  ...
  <div class="searchBar">
    <label
      >站點名稱搜尋：<input
        type="text"
        :value="stopName"
        @input="updateHandler('stopName', $event)"
    /></label>
    <label
      >行政區篩選：
...
export default {
  ...
  methods: {
    updateHandler(key, e) {
      if (key === "available")
        return this.sendSearchFilter(key, !this.available);

      let value = e.target.value;
      value =
        key === "count" || key === "pageSize" ? Number(value) : value.trim();
